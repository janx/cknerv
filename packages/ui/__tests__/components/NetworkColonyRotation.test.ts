...
describe('NetworkColony counter-rotation frame contract', () => {
  ...
  it('rotates the edges, cohort holes, nodes and the inspection overlay; leaves courier + delivery in world space', () => {
    const network = source('NetworkColony.tsx');
    ...
    // ⚠️ AND THE ENERGY FIELD HAS NO LAYER OF ITS OWN TO MOUNT. A `ColonyMist`
    // stood here first, drawing up to two ambient sheets under the whole
    // colony; it was removed on 2026-09-02 after a live leg measured a sheet's
    // own brightest pixel anywhere on the canvas at 2/255 while it cost 0.90 ms
    // of the layer's 1.06 ms of frame GPU at the app camera. The whole mist is
    // now the intake patch inside `ColonyCohorts`, off that layer's own plan
    // and its own two lanes, so there is no second planner and no prop to keep
    // in step — and the patch rides this group for the reason the sheets did
    // not need to: its sink is a colony-frame constant.
    expect(network).not.toContain('ColonyMist');
  });
