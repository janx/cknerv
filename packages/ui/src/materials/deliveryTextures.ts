...
/**
 * A protocol landing seal: two counter-rotated interrupted agreement loops,
 * six contributor nodes, and a compact central braid knot. It replaces the
 * generic shockwave annulus with the same grammar used by Cell write seals.
 */
export function makeProtocolLandingTexture(): THREE.Texture {
  const size = 128;
  const canvas = document.createElement('canvas');
  canvas.width = size;
  canvas.height = size;
  ...
  strokeInterruptedLoop(ctx, c, c, 39, 29, 0.08, 0.31, 0.92, 2.2);
  strokeInterruptedLoop(ctx, c, c, 28, 39, 0.44, -0.39, 0.6, 1.5);

  ctx.fillStyle = 'rgba(255,255,255,0.92)';
  for (let node = 0; node < 6; node += 1) {
    const angle = node / 6 * Math.PI * 2 + 0.12;
    const x = c + Math.cos(angle) * 42;
    const y = c + Math.sin(angle) * 33;
    ctx.fillRect(x - 1.75, y - 1.75, 3.5, 3.5);
  }

  // Central four-way agreement knot. The two open diamonds read as a woven
  // crossing instead of a solid badge or architectural emblem.
  ctx.strokeStyle = 'rgba(255,255,255,0.96)';
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(c, c - 10);
...
/** Compatibility export for downstream consumers; now returns the A seal. */
export function makeRingTexture(): THREE.Texture {
  return makeProtocolLandingTexture();
}
