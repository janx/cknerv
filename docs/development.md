...
The home-screen icon works the same way, from `ui-app/app-icon.svg`, because
iOS accepts no SVG for `apple-touch-icon`. Three sizes, all from the one
source — 180 for iOS, 192 and 512 for the install manifest:

```bash
for size in 180:apple-touch-icon 192:app-icon-192 512:app-icon-512; do
  chromium --headless=new --no-sandbox --hide-scrollbars \
    --force-device-scale-factor=1 --window-size="${size%%:*},${size%%:*}" \
    --default-background-color=02030aff --virtual-time-budget=3000 \
    --screenshot="$PWD/ui-app/public/${size##*:}.png" \
    "file://$PWD/ui-app/app-icon.svg"
done
```

`ui-app/public/manifest.webmanifest` is generated by `pnpm brand:assets` and
declares `display: standalone`: added to a home screen the dashboard owns the
screen outright, with no browser chrome over the top bar. `index.html` pairs it
with the Apple meta for iOS versions that predate manifest `display` support,
and asks for a translucent status bar — which is only safe because the HUD
frame stands inside `env(safe-area-inset-*)`.
