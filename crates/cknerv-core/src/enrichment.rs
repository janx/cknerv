...
///
/// [`PeerAdvertisedEvidence`] is what reads it: the dossier names the furthest
/// rung one peer's dials reached, and it finds that rung by taking the maximum
/// of this enum's own `Ord`. So the declaration order below is not a matter of
/// taste — it IS the axis, and [`Self::HANDSHAKE_AXIS`] writes it out where a
/// reader can check the sentence it spells.
...
impl PeerProbeResult {
    /// The six results the crawler can actually name, weakest rung first.
    ///
    ...
    ///
    /// That pairing is the whole job. The dossier picks a peer's furthest rung
    /// with `max` over this `Ord`, which is only the right answer while the
    /// `Ord` spells the handshake; a derive gives no warning when a variant is
    /// moved, and this constant plus its test is what does.
    ///
    /// [`Self::Unknown`] is deliberately absent. It is cknerv's own word for
    /// an observation it could not read, ranked below every result it can
    /// name so that any nameable one on any of a peer's addresses wins the
    /// `max` — which is a statement about the six, not a seventh member of
    /// them.
...
/// is exactly `verified_retained_peers`. The first of those is what lets the
/// panel draw the three outcomes as shares of one bar; the second is why
...
/// yet, and a state this build has no name for. They are counted on the
/// atlas's reach bar where a count is what they are worth, and the roster
/// names nobody it
...
pub struct PeerAdvertisedEvidence {
    ...
    /// socket yields no address to keep. Absent rather than filled from the
    /// clock below, because "the network last named it" and "something last
    /// observed it" are different sentences and only one of them is this
    /// field.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub last_advertised_at_ms: Option<u64>,
    /// When anything at all last observed this peer — the newest of every
    /// positive channel the crawler holds: a gossiped alias, evidence naming
    /// it as a target, a direct session, or the crawler's own identification.
    ///
    /// ⭐ THE CLOCK THAT MAKES THE REPORT DATABLE, and it took that job from
    /// the advertise clock above for the same reason the roster's did. An
    /// unverified peer has no sighting to be stamped by, and an undated
    /// statement about the network is the one thing the DOSSIER never prints —
    /// so the required clock has to be one the source can always answer, and
    /// the source guarantees this one: a candidate with no positive evidence
    /// to take a maximum over is a request it refuses outright rather than a
    /// record it dates with `null`.
    pub latest_positive_observed_ms: u64,
...
mod tests {
    ...
    /// sentence it spells, and the DOSSIER picks a peer's furthest rung by
    /// this enum's `Ord` — a `max` that is only the right answer while the
    /// declaration order IS that sentence. Moving a variant is a one-line edit
    /// that changes which failure the plate reports and warns about nothing,
    /// so the hand-written list and the derived order are held against each
    /// other here.
    ///
    ...
    fn handshake_axis_is_this_enum_in_order() {
        ...
        // …and that it is every result the crawler can name, with cknerv's own
        // word for one it cannot ranked below all of them — which is what makes
        // the plate's `max` fall back to `Unknown` only when nothing else was
        // readable, rather than letting it win a tie.
